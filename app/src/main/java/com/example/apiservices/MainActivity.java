    ...
    public void parserJson(JSONObject response){
        try {
            ...
            for (int i = 0 ; i < competitions.length(); i++) {
                ...
                String nomarea = area.getString("name"); // ← ahora es "web"
                String fundado = com.optString("founded", "N/A"); // ← fundado

                cadena = cadena + id + "," + nomcomp + "," + nomarea + "," + fundado + "," + code + "\n";
                Competition co = new Competition(id, nomcomp, nomarea, fundado, code); // ← CORRECTO
                cs.add(co);
            }
            json.setText(cadena);
